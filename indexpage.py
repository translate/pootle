...
class ProjectIndex(pagelayout.PootleNavPage):
    ...
    def __init__(self, translation_project, request, directory):
        ...
        # handle actions before generating URLs, so we strip
        # unneccessary parameters out of argdict
        self.handleactions(request)
        # generate the navigation bar maintaining state
        directory = translation_project.directory_root
        navbarpath_dict = self.makenavbarpath_dict(project=translation_project,
                request=self.request, directory=directory)
        ...
        if get_bool(request.GET, 'editing'):
            actionlinks = self.getactionlinks(directory, [
                'editing',
                'mine',
                'review',
                'check',
            ...
            actionlinks = self.getactionlinks(directory, ['editing', 'goal',
                    'zip', 'sdf'])
        ...
        # l10n: The first parameter is the name of the installation
        # (like "Pootle")
        pagetitle = _('%s: Project %s, Language %s' % (instancetitle,
                      project.fullname, tr_lang(language.fullname)))
        ...
        if 'admin' in self.rights or 'translate' in self.rights or 'suggest'\
             in self.rights:
            self.templatevars['upload'] = self.getuploadbox()

    def handleactions(self, request):
        """handles the given actions that must be taken (changing
        operations)"""
        ...
        if 'doassign' in self.argdict:
            ...
            assigncount = self.translation_project.assignpoitems(self.request,
                    search, assignto, action)
            print 'assigned %d strings to %s for %s' % (assigncount, assignto,
                    action)
        ...
        if self.getboolarg('removeassigns'):
            ...
            assigncount = \
                self.translation_project.unassignpoitems(self.request, search,
                    assignedto)
        ...
        if 'doupload' in self.argdict:
            ...
            if transfiles:
                self.translation_project.uploadfile(self.request, self.dirname,
                        uploadfile.filename, uploadfile.contents, overwrite)
                self.translation_project.scanpofiles()
            elif uploadfile.filename.endswith('.zip'):
                self.translation_project.uploadarchive(self.request,
                        self.dirname, uploadfile.contents, overwrite)
                self.translation_project.scanpofiles()
            else:
        ...
        if 'doupdate' in self.argdict:
            ...
            if updatefile.endswith('.' + self.translation_project.fileext):
                self.translation_project.updatepofile(self.request,
                        self.dirname, updatefile)
                self.translation_project.scanpofiles()
            else:
                raise ValueError('can only update files with extension .'
                                  + self.translation_project.fileext)
        ...
        if 'docommit' in self.argdict:
            ...
            if commitfile.endswith('.' + self.translation_project.fileext):
                self.translation_project.commitpofile(self.request,
                        self.dirname, commitfile)
            else:
                raise ValueError('can only commit files with extension .'
                                  + self.translation_project.fileext)
        ...
        if 'doaddgoal' in self.argdict:
            ...
            self.translation_project.setgoalfiles(self.request,
                    goalname.strip(), '')
        ...
        if 'doeditgoal' in self.argdict:
            ...
            if not isinstance(goalnames, list):
                goalnames = [goalnames]
            goalnames = [goalname.strip() for goalname in goalnames
                         if goalname.strip()]
            self.translation_project.setfilegoals(self.request, goalnames,
                    goalfile)
        ...
        if 'doeditgoalusers' in self.argdict:
            ...
            if addusername:
                self.translation_project.addusertogoal(self.request, goalname,
                        addusername)
        ...
        if 'doedituser' in self.argdict:
            ...
            if not isinstance(goalusers, list):
                goalusers = [goalusers]
            goalusers = [goaluser.strip() for goaluser in goalusers
                         if goaluser.strip()]
            if not isinstance(goalnames, list):
                goalnames = [goalnames]
            goalnames = [goalname.strip() for goalname in goalnames
            ...
            for goalname in goalnames:
                ...
                self.translation_project.reassignpoitems(self.request, search,
                        goalusers, action)
            del self.argdict['doedituser']
        # pop arguments we don't want to propogate through
        # inadvertently...
        for argname in (
            'assignto',
            'action',
    ...
    def getgoalbox(self):
        ...
        return {'title': _('goals'), 'name-title': _('Enter goal name'),
                'button': _('Add Goal')}

    def getuploadbox(self):
        """adds a box that lets the user assign strings"""

        uploadbox = {'title': _('Upload File'),
    ...
    def getchilditems(self, directory, goal):
        """get all the items for directories and files viewable at
        this level"""
        ...
        childitems = [diritem for (childdir, diritem) in diritems] + [fileitem
                for (childfile, fileitem) in fileitems]
    ...
    def getgoalitems(self, directory):
        ...
        def get_all_goals(directory):
            if directory is not None:
                return list(directory.goal_set.all())\
                     + get_all_goals(directory.parent)
            else:
                return []
        ...
        for goalname in goalnames:
            ...
            if self.currentgoal == goalname:
                goalchilditems = self.getitems(goalfiles,
                        linksrequired=['editgoal'], goal=self.currentgoal)
    ...
    def getgoalitem(self, goal_obj, dirfilter, goalusers):
        ...
        if pofilenames:
            actionlinks = self.getactionlinks('', linksrequired=['mine',
                    'review', 'translate', 'zip'], goal=goalname)
            goal['actions'] = actionlinks
        goaluserslist = []
        if goalusers:
            goalusers.sort()
            goaluserslist = [{'name': goaluser, 'sep': ', '} for goaluser in
                             goalusers]
            if goaluserslist:
                goaluserslist[-1]['sep'] = ''
        ...
        if goalname and self.currentgoal == goalname:
            if 'admin' in self.rights:
                unassignedusers = \
                    self.translation_project.getuserswithinterest()
    ...
    def getdiritem(self, directory, goal, linksrequired=None, **newargs):
        ...
        diritem = {
            'href': browseurl,
            'title': directory.pootle_path,
            'icon': 'folder',
            'isdir': True,
            }
        actionlinks = self.getactionlinks(directory,
                linksrequired=linksrequired)
        diritem['actions'] = actionlinks
        if self.showgoals and 'goal' in self.request.GET:
            diritem['stats'] = self.getitemstats(directory, self.currentgoal,
                    (directory.num_stores(self.currentgoal),
                    directory.num_stores()))
        else:
            diritem['stats'] = self.getitemstats(directory, self.currentgoal,
                    directory.num_stores())
        diritem['data'] = self.getstats(self.translation_project, directory,
                                        goal)
        return diritem
    ...
    def getfileitem(self, store, linksrequired=None, **newargs):
        ...
        if 'mo' in linksrequired:
            if self.translation_project.db_translation_project.project.createmofiles\
                 and 'pocompile' in self.rights:
        ...
        if 'update' in linksrequired and 'admin' in self.rights:
            if versioncontrol.hasversioning(os.path.join(self.translation_project.podir,
                ...
                # l10n: Update from version control (like CVS or
                # Subversion)
                updatelink = \
                    {'href': 'index.html?editing=1&doupdate=1&updatefile=%s'\
                      % store.name, 'text': _('Update')}
        ...
        if 'commit' in linksrequired and 'commit' in self.rights:
            if versioncontrol.hasversioning(os.path.join(self.translation_project.podir,
                ...
                commitlink = \
                    {'href': 'index.html?editing=1&docommit=1&commitfile=%s'\
                      % store.name, 'text': _('Commit')}
        ...
        fileitem['data'] = self.getstats(self.translation_project,
                self.translation_project.getquickstats([fileentry]))
        return fileitem
    ...
    def getgoalform(self, basename, goalfile, filegoals):
        ...
        if len(useroptions) > 1:
            assignfilenames = \
                self.translation_project.browsefiles(dirfilter=goalfile)
            if self.currentgoal:
                action = 'goal-' + self.currentgoal
            else:
                action = None
            assignstats = \
                self.translation_project.combineassignstats(assignfilenames,
                    action)
            assignusers = list(assignstats.iterkeys())
            useroptions += [username for username in assignusers if username
                             not in useroptions]
            if len(assignusers) > 1:
                multiusers = 'multiple'
            assignwhich = [('all', _('All Strings')), ('untranslated',
                           _('Untranslated')), ('unassigned', _('Unassigned')),
                           ('unassigneduntranslated',
                           _('Unassigned and Untranslated'))]
        return {
            'name': goalformname,
            'filename': basename,
            'goalnames': goalnames,
            'filegoals': dict([(goalname, goalname in filegoals or None)
                              for goalname in goalnames]),
            'multifiles': multifiles,
            'setgoal_text': _('Set Goal'),
            'users': useroptions,
            'assignusers': dict([(username, username in assignusers or None)
                                for username in useroptions]),
            'multiusers': multiusers,
            'selectmultiple_text': _('Select Multiple'),
            'assignwhich': [{'value': value, 'text': text} for (value, text) in
                            assignwhich],
            'assignto_text': _('Assign To'),
            }

    def getactionlinks(self, path_obj, linksrequired=None, filepath=None,
                       goal=None):
        """get links to the actions that can be taken on an item
        (directory / file)"""
        ...
        stats_totals = \
            path_obj.get_stats_totals(self.translation_project.checker)
        ...
        def addoptionlink(linkname, rightrequired, attrname, showtext,
                          hidetext):
            if linkname in linksrequired:
                if rightrequired and not rightrequired in self.rights:
                    return
                if getattr(self, attrname, False):
                    link = {'href': self.makelink(baseindexlink,
                            **{attrname: 0}), 'text': hidetext}
                else:
                    link = {'href': self.makelink(baseindexlink,
                            **{attrname: 1}), 'text': showtext}
                link['sep'] = ' | '
                actionlinks.append(link)

        addoptionlink('editing', None, 'editing', _('Show Editing Functions'),
                      _('Show Statistics'))
        # l10n: "Checks" are quality checks that Pootle performs on
        # translations to test for common mistakes
        addoptionlink('check', 'translate', 'showchecks', _('Show Checks'),
                      _('Hide Checks'))
        addoptionlink('goal', None, 'showgoals', _('Show Goals'), _('Hide Goals'
                      ))
        addoptionlink('assign', 'translate', 'showassigns', _('Show Assigns'),
                      _('Hide Assigns'))
        actions['basic'] = actionlinks
        actionlinks = []
        if not goal:
            filegoals = \
                self.translation_project.getfilegoals(path_obj.pootle_path)
            if self.showgoals:
                if len(filegoals) > 1:
                    # TODO: This is not making sense. For now make it
                    # an unclickable link
                    allgoalslink = {'href': '', 'text': _('All Goals: %s'
                                     % ', '.join(filegoals))}
                    actionlinks.append(allgoalslink)
            if 'editgoal' in linksrequired and 'admin' in self.rights:
                actions['goalform'] = self.getgoalform(basename, goalfile,
                        filegoals)
        if 'mine' in linksrequired and not self.request.user.is_anonymous:
            ...
            if len(mystats):
                minelink = {'href': self.makelink(baseactionlink,
                            assignedto=self.request.user.username),
                            'text': minelink}
            else:
                minelink = {'title': _('No strings assigned to you'),
                            'text': minelink}
            actionlinks.append(minelink)
            if 'quick' in linksrequired and 'translate' in self.rights:
                if len(mystats) > 0:  # A little shortcut to avoid the call to
                                      # projectstats.units if we don't have
                                      # anything assigned
                    mytranslatedstats = [statsitem for statsitem in mystats
                            if statsitem in stats.units.get('translated', [])]
                else:
                    mytranslatedstats = []
                quickminelink = _('Quick Translate My Strings')
                if len(mytranslatedstats) < len(mystats):
                    quickminelink = {'href': self.makelink(baseactionlink,
                                     assignedto=self.request.user.username,
                                     fuzzy=1, untranslated=1),
                                     'text': quickminelink}
                else:
                    quickminelink = \
        ...
        if 'review' in linksrequired and stats_totals.get('check-hassuggestion'
                , 0):
            if 'review' in self.rights:
                reviewlink = _('Review Suggestions')
            else:
                reviewlink = _('View Suggestions')
            reviewlink = {'href': self.makelink(baseactionlink, review=1,
                          **{'hassuggestion': 1}), 'text': reviewlink}
        ...
        if 'quick' in linksrequired:
            ...
            if stats_totals['translated'] < stats_totals['total']:
                quicklink = {'href': self.makelink(baseactionlink, fuzzy=1,
                             untranslated=1), 'text': quicklink}
            else:
                quicklink = {'title': _('No untranslated items'),
                             'text': quicklink}
            actionlinks.append(quicklink)
        if 'all' in linksrequired and 'translate' in self.rights:
            translatelink = {'href': self.makelink(baseactionlink),
        ...
        if 'zip' in linksrequired and 'archive' in self.rights:
            ...
            archivename = '%s-%s' % (self.translation_project.projectcode,
                                     self.translation_project.languagecode)
            if currentfolder:
            ...
            ziplink = {'href': archivename, 'text': linktext,
                       'title': archivename}
            actionlinks.append(ziplink)
        if 'sdf' in linksrequired and 'pocompile' in self.rights\
             and self.translation_project.ootemplate() and not (basename
                 or filepath):
            archivename = self.translation_project.languagecode + '.sdf'
            linktext = _('Generate SDF')
            oolink = {'href': archivename, 'text': linktext,
                      'title': archivename}
        ...
        if not actions['extended'] and not actions['goalform']\
             and actions['basic']:
            actions['basic'][-1]['sep'] = ''
        return actions
    ...
    def getitemstats(self, path_obj, goal, numfiles, url_opts={}):
        ...
        stats = {
            'summary': self.describestats(self.translation_project, path_obj,
                    goal, numfiles),
            'checks': [],
            'tracks': [],
            'assigns': [],
        ...
        if self.showchecks:
            stats['checks'] = self.getcheckdetails(path_obj, goal, linkbase,
                    url_opts)
        if self.showassigns:
            ...
            stats['assigns'] = self.getassigndetails(path_obj, linkbase,
                    removelinkbase)
        return stats
    ...
    def getcheckdetails(self, path_obj, goal, linkbase, url_opts={}):
        ...
        property_stats = \
            path_obj.get_property_stats(self.translation_project.checker, goal)
        ...
        for checkname in keys:
            ...
            if total and checkcount:
                stats = nlocalize('%d string (%d%%) failed',
                                  '%d strings (%d%%) failed', checkcount,
                                  checkcount, (checkcount * 100) / total)
                url_opts[str(checkname)] = 1
                checklink = {'href': self.makelink(linkbase, **url_opts),
                             'text': checkname, 'stats': stats}
    ...
    def getassigndetails(self, pofilenames, linkbase, removelinkbase):
        ...
        # TODO: allow setting of action, so goals can only show the
        # appropriate action assigns quick lookup of what has been
        # translated
        ...
        for assignname in keys:
            ...
            complete = [statsitem for statsitem in assigned if statsitem
                         in projectstats.units.get('translated', [])]
            completecount = len(complete)
            completewords = self.translation_project.countwords(complete)
            if totalcount and assigncount:
                assignlink = {'href': self.makelink(linkbase,
                              assignedto=assignname), 'text': assignname}
                percentassigned = (assignwords * 100) / max(totalwords, 1)
                percentcomplete = (completewords * 100) / max(assignwords, 1)
                stats = _('%d/%d words (%d%%) assigned' % (assignwords,
                          totalwords, percentassigned))
                stringstats = _('[%d/%d strings]' % (assigncount, totalcount))
                completestats = _('%d/%d words (%d%%) translated'
                                   % (completewords, assignwords,
                                  percentcomplete))
                completestringstats = _('[%d/%d strings]' % (completecount,
                                        assigncount))
                if 'assign' in self.rights:
                    removetext = _('Remove')
                    removelink = {'href': self.makelink(removelinkbase,
                                  assignedto=assignname), 'text': removetext}
                else:
                    removelink = None
                assignlinks.append({
                    'assign': assignlink,
