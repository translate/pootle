...
urlpatterns = patterns('pootle_store.views',
    ...
    (r'^(?P<pootle_path>.*)/view/(?P<uid>[0-9]+)/?$', 'get_view_unit'),
    (r'^(?P<pootle_path>.*)/view/for/(?P<uid>[0-9]+)/?$', 'get_view_units_for'),
    (r'^(?P<pootle_path>.*)/view/for/(?P<uid>[0-9]+)/limit/(?P<limit>[0-9]+)/?$', 'get_view_units_for'),
    (r'^(?P<pootle_path>.*)/edit/(?P<uid>[0-9]+)/?$', 'get_edit_unit'),
    (r'^(?P<pootle_path>.*)/process/(?P<uid>[0-9]+)/submission/?$', 'process_submission'),
