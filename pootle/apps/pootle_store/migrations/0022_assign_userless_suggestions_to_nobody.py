...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        suggestions = orm.Suggestion.objects.filter(user__isnull=True)
        try:
            nobody = orm['accounts.User'].objects.get(username='nobody')
        except ObjectDoesNotExist:
            logging.debug("'nobody' user is not present, so suggestions with "
                          "no user can't be assigned to 'nobody'. This only "
                          "happens if Pootle is being migrated from scratch, "
                          "like in Travis. Pootle 2.6 is meant to upgrade "
                          "existing deployments, so you should never see this "
                          "message.")
        else:
