...
from evernote_auth.models import EvernoteAccount

from pootle_misc.baseurl import redirect, get_next
    ...
    if d:   
        ...
        if len(ea) == 0:
            if not create:
                ...
                return redirect('/accounts/evernote/login/link?%s' % urlencode({
                                auth.REDIRECT_FIELD_NAME: redirect_to})) 
                #    'create': 1, 
                #   'name': d['name'], 
                #    'email': d['email'], 
                #    'username': pootle_username
                #    }))
...
def evernote_login(request, create=0):
    ...
    script_name = "%s/" % settings.SCRIPT_NAME.rstrip('/').lstrip('/') if settings.SCRIPT_NAME else ''
    ...
    if not request.user.is_authenticated():
        if create:
            ...
            return redirect(settings.EVERNOTE_LOGIN_URL + '%s/%saccounts/evernote/return/%s' % (server_alias, script_name, redirect_to.lstrip('/')))
    else:
        if not hasattr(request.user, 'evernote_account'):
            return redirect(settings.EVERNOTE_LOGIN_URL + '%s/%saccounts/evernote/create/return/%s' % (server_alias, script_name, redirect_to.lstrip('/')))
        else:
            return redirect_after_login(request, redirect_to)
    ...
    if request.user.is_authenticated():
        ...
        if request.POST:
            ...
            if form.is_valid():
                ...
                ea = EvernoteAccount.objects.filter(**{'evernote_id': d['id']})
                if len(ea) == 0:
                    ea = EvernoteAccount(evernote_id = d['id'], email=d['email'], name=d['name'])
                    ea.user = request.user
                    ea.save()
