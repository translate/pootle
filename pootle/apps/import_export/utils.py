...
def import_file(file, user=None):
    ...
    if not hasattr(f, "parseheader"):
        raise UnsupportedFiletypeError(ERR_UNSUPPORTED_FILETYPE % file.name)
    header = f.parseheader()
    pootle_path = header.get("X-Pootle-Path")
    if not pootle_path:
        raise MissingPootlePathError(ERR_MISSING_POOTLE_PATH % file.name)

    rev = header.get("X-Pootle-Revision")
    if not rev or not rev.isdigit():
        raise MissingPootleRevError(ERR_MISSING_POOTLE_REV % file.name)
    rev = int(rev)

    try:
        store, created = Store.objects.get_or_create(pootle_path=pootle_path)
    except Exception as e:
        raise FileImportError(ERR_FILE_IMPORT % (file.name, e))
