...
ANN_VPATH = u'announcements/'


class Migration(DataMigration):

    def forwards(self, orm):
        announcements = orm.StaticPage.objects.filter(
            virtual_path__startswith=ANN_VPATH
        )

        for ann in announcements:
            project_code = ann.virtual_path[len(ANN_VPATH):]
            ann.virtual_path = u'announcements/projects/' + project_code
            ann.save()

    def backwards(self, orm):
        announcements = orm.StaticPage.objects.filter(
            virtual_path__startswith=u'announcements/projects/'
        )

        for ann in announcements:
            project_code = ann.virtual_path[len(u'announcements/projects/'):]
            ann.virtual_path = ANN_VPATH + project_code
            ann.save()
