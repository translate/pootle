...
- With editable project and language descriptions you can supply description
  for projects. These are editable using Markdown, reStructuredText or HTML.
...
- :setting:`--modified-since` flag for `update_stores
...
- ``update_from_templates`` has been renamed to `update_against_templates
  <http://docs.translatehouse.org/projects/pootle/en/stable-2.5.0/server/commands.html#update-against-templates>`_
...
- Make sure to use the minimum required South version when performing
  database upgrades.
